...
cv::Mat CreateDepthImage(cv::Mat L_img, cv::Mat R_img){
  ...
  //Masking unused borders
	...
	if (!frameBuffer_sgbm.empty()){
		...
		masked_sgbm = fovReduction(masked_sgbm);
		DepthProcessing(masked_sgbm);
...
void DepthProcessing(cv::Mat src_img){
	...
	const static float FOV_x = float(src_img.cols/float(WIDTH))*float(cameraFOVx);
	const static float FOV_y = float(src_img.rows/float(HEIGHT))*cameraFOVy;
