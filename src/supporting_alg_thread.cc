...
void maskOutliers(int thread_id, const cv::Mat& src_img, cv::Mat& dst_img, const cv::Mat& prevFrame, const int nFrames, const float diffThreshold){
  ...
  maskBuffer[thread_id].push_front(mask);
  if (maskBuffer[thread_id].size() == nFrames+1){
    maskBuffer[thread_id].pop_back();
  }

  for(std::list<cv::Mat>::iterator it = maskBuffer[thread_id].begin(); it != maskBuffer[thread_id].end(); it++){
    cv::Mat buf = *it;
