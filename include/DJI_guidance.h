...
*     @return `error code`. Non-zero if error occurs.
*/
SDK_API int get_stereo_cali( stereo_cali stereo_cali_pram[CAMERA_NUM]);

/**
*	@fn int get_online_status(int online_status[CAMERA_NUM]);
*	@brief Get the online status of Guidance sensors.
*	@param `online_status` Array of online status for all sensors.
*	@return  `error code`. Non-zero if error occurs.
*/
SDK_API int get_online_status(int online_status[CAMERA_NUM]);
...
/**
*	@fn int set_exposure_param( auto_exp_param param[CAMERA_NUM] );
